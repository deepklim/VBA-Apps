...
Sub Geocode()
    ...
    WS.Range("C1") = "Latitude"
    WS.Range("D1") = "Longitude"
    
    'Pass to API
    Dim i As Long
    Dim split_result As Variant
    Dim city_state As String, query_result As String
    
    For i = 2 To r
        city_state = WS.Range("A" & i) & ", " & WS.Range("B" & i)
        query_result = LatLong(city_state)
        'If query successful, split into lat & lng, else return error
        If InStr(query_result, ",") Then
            split_result = Split(query_result, ",")
            WS.Range("C" & i) = split_result(0)
            WS.Range("D" & i) = split_result(1)
        Else
            WS.Range("C" & i) = query_result
        End If
        'Sleep for 200 miliseconds to avoid 'OVER_QUERY_LIMIT' error
        Sleep 200
    Next
